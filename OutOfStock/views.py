...
from .forms import TideForm, FaceNetForm
...
mod=load_model('OutOfStock/lstm_model.h5')




def stock_predictor(request):
    x = datetime.datetime.now()
    d=int(x.strftime("%d"))

    if request.method == "POST":
        form = TideForm(request.POST)

        if form.is_valid():
            inventory = form.cleaned_data['current_inventory']
            day = 0
            while(inventory>=0):
                ...
                if(inventory<=0):
                    return render(request, 'OutOfStock/tide.html', {'day': str(day) + ' days'})
    else:
        form = TideForm()

    return render(request, 'OutOfStock/tide.html', {'form':form})
